...
public:
	explicit constexpr ReissnerNordstromMetric(
		Scalar mass,
		Scalar charge,
	...
	[[nodiscard]] constexpr bool is_subextremal() const noexcept {
		return (r_s_ * r_s_) > (static_cast<Scalar>(4) * r_q2_);
	}
	...
	[[nodiscard]] Scalar outer_horizon_radius() const noexcept {
		const Scalar discr = r_s_ * r_s_ - static_cast<Scalar>(4) * r_q2_;
		if (discr < static_cast<Scalar>(0)) {
			return static_cast<Scalar>(0.5) * r_s_;
		}
		return static_cast<Scalar>(0.5) * (r_s_ + std::sqrt(discr));
	}
	...
	[[nodiscard]] Scalar outer_photon_sphere_radius() const noexcept {
		const Scalar discr = static_cast<Scalar>(9) * r_s_ * r_s_ - static_cast<Scalar>(32) * r_q2_;
		if (discr < static_cast<Scalar>(0)) {
			return static_cast<Scalar>(0.75) * r_s_;
		}
		return (static_cast<Scalar>(3) * r_s_ + std::sqrt(discr)) / static_cast<Scalar>(4);
	}

	[[nodiscard]] Scalar inner_photon_sphere_radius() const noexcept {
		const Scalar discr = static_cast<Scalar>(9) * r_s_ * r_s_ - static_cast<Scalar>(32) * r_q2_;
		if (discr < static_cast<Scalar>(0)) {
			return static_cast<Scalar>(0.75) * r_s_;
		}
	...
	[[nodiscard]] Core::MetricTensor<Scalar> metric_tensor(const Core::FourVector<Scalar>& x) const noexcept {
		...
		const Scalar factor = static_cast<Scalar>(1) - r_s_ / r + r_q2_ / (r * r);
	...
	[[nodiscard]] Core::ChristoffelSymbols<Scalar> christoffel_symbols(const Core::FourVector<Scalar>& x) const noexcept {
		...
		const Scalar factor = static_cast<Scalar>(1) - r_s_ / r + r_q2_ / r2;
		const Scalar df_dr = (r_s_ * r - static_cast<Scalar>(2) * r_q2_) / r3;
