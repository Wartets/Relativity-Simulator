...
- Zero Dynamic Allocations in Execution Loops: Computational loops operate within pre-allocated linear memory arenas (`LinearMemoryArena`) aligned to 64-byte and 128-byte cache boundaries.
...
- Strict Numerical Determinism: Trajectory calculations, N-body evolutions, & stochastic sampling use explicit-state PCG64 generators (`PCG64Engine`). Floating-point arithmetic rounding behavior is preserved to maintain identical results across runs.
...
- `Scheduler`: Deterministic fixed-step clock engine supporting dynamic time-warp factors, pause/step functionality, & real-time accumulator regulation.
- `SimulationOrchestrator`: Coordinates physics processing, parameter updates, camera state management, & thread communication over lock-free single-producer single-consumer queues (`SpscQueue`).
- `InteractiveCameraController`: Manages Free-Fly 6-DOF, Orbit Center, Spherical Boyer-Lindquist, and Cockpit Flight navigation modes across 8 camera projection models.
- `GeodesicComputePipeline` & `SoftwareComputeEngine`: Multithreaded software compute engine executing native double-precision (FP64) & Double-Single (`fp32/fp32`) emulation for camera ray integration.
- `UiManager`: Coordinates GLFW windowing, ImGui docking workspaces, multi-viewport separation, & secondary camera views.
- Telemetry Windows:
  - `ViewportPrimaryWindow`: Displays the raytraced image buffer, interactive HUD overlay, & transport controls.
  - `ScenarioSelectorWindow`: Scenario catalog loader & YAML parser.
  - `ControlPanelWindow`: Parameters for metrics, camera properties, integrators, & rocket thrust.
  - `TelemetryWindow`: Curvature scalars (Ricci $R$, Kretschmann $K_1$), Hamiltonian residuals, & coordinate telemetry.
