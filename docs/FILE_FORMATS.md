...
Scenarios are defined in YAML format and specify initial spacetime properties, numerical integrator settings, observer definitions, and celestial bodies.
...
output:
  fits_enabled: true                      # Export FITS images and spectral cubes
...
observers:
  - name: "PrimaryObserver"
    ...
    position: [0.0, 50.0, 1.5707963, 0.0] # Observer coordinates [t, r, theta, phi]
...
Images and spectral cubes generated by the raytracing engine comply with the NASA/IAU FITS Standard 4.0. Pixel arrays use 64-bit IEEE 754 floating-point values (`BITPIX = -64`) stored in big-endian byte order, formatted in 2880-byte logical records.
...
- `CRPIX3`, `CRVAL3`, `CDELT3`: Reference wavelength channel, central wavelength, and spectral channel width in meters (`CUNIT3 = 'm'`, `CTYPE3 = 'WAVE'`).

---

## 3. Hierarchical Data Format (`.h5`)

Dense trajectories, metric tensor series, and uncertainty quantities are stored in containerized HDF5 binary files.
...
- `PathLength` (4 bytes) and `PathString` (ASCII path string).
...
| :--- | :--- | :--- | :--- |
| `/bodies/<name>/position` | `Float64` | $[N \times 4]$ | Spacetime coordinates $[t, x, y, z]$ along worldline. |
| `/bodies/<name>/velocity` | `Float64` | $[N \times 4]$ | Four-velocity components $[u^0, u^1, u^2, u^3]$. |
...
Geometry exports of horizons and geodesic trajectories use VTK XML PolyData format in little-endian byte ordering.
...
Event horizons and photon spheres export with `<Polys>` connectivity structures defining quadrilateral and triangular surface elements over parameterized spherical coordinates $(\theta, \phi)$.

---

## 5. Planetary Ephemeris & SPICE Integration

The engine ingests planetary state vectors and orbital elements from NASA/JPL data formats.

### 5.1. NASA JPL HORIZONS API Responses

CSV vector tables are parsed between the `$$SOE` and `$$EOE` markers:
...
Chebyshev polynomial position and velocity segments are evaluated through recurrence relations:
...
- Type 3: Independent Chebyshev polynomial sets for position and velocity coordinates.
