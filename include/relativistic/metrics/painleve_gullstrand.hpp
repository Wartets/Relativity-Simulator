...
class PainleveGullstrandMetric {
private:
	Scalar mass_;
	Scalar c_;
	Scalar g_const_;
...
public:
	explicit constexpr PainleveGullstrandMetric(
		Scalar mass,
	...
	[[nodiscard]] Core::MetricTensor<Scalar> metric_tensor(const Core::FourVector<Scalar>& x) const noexcept {
		...
		const Scalar sqrt_rs_r = std::sqrt(r_s_ / r);
	...
	[[nodiscard]] Core::MetricTensor<Scalar> inverse_metric(const Core::FourVector<Scalar>& x) const noexcept {
		...
		inv_g(0, 1) = sqrt_rs_r / c_;
		inv_g(1, 0) = sqrt_rs_r / c_;
	...
	[[nodiscard]] Core::ChristoffelSymbols<Scalar> christoffel_symbols(const Core::FourVector<Scalar>& x) const noexcept {
		...
		const Scalar g000 = (c_ * r_s_ * sqrt_rs_r) / (static_cast<Scalar>(2) * r2);
		gamma(0, 0, 0) = g000;
		gamma(0, 0, 1) = r_s_ / (static_cast<Scalar>(2) * r2);
		gamma(0, 1, 0) = gamma(0, 0, 1);
		gamma(0, 1, 1) = sqrt_rs_r / (static_cast<Scalar>(2) * c_ * r);
		gamma(0, 2, 2) = -(r * sqrt_rs_r) / c_;
		gamma(0, 3, 3) = gamma(0, 2, 2) * sin_t * sin_t;

		gamma(1, 0, 0) = (c_ * c_ * r_s_ * (r - r_s_)) / (static_cast<Scalar>(2) * r3);
		gamma(1, 0, 1) = -g000;
		gamma(1, 1, 0) = gamma(1, 0, 1);
		gamma(1, 1, 1) = -r_s_ / (static_cast<Scalar>(2) * r2);
		gamma(1, 2, 2) = -(r - r_s_);
